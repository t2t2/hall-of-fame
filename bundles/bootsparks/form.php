...
class Form extends \Laravel\Form {
	...
	public static function field($type, $name, $label, array $args = array(), array $opts = array())
	{
		...
		if(!empty($opts["prepend"]) || !empty($opts["append"]))
		{
			$classes = array_intersect(array('prepend', 'append'), array_keys($opts));
			array_walk($classes, function(&$value) { $value = "input-{$value}"; });
			$out .= '<div class="'.implode(' ', $classes).'">';
			if(!empty($opts["prepend"]))
			{
				$out .= '<span class="add-on">'.$opts["prepend"].'</span>';
			}
			$out .= forward_static_call_array(array('Form', $type), $args);
			if(!empty($opts["append"]))
			{
				$out .= '<span class="add-on">'.$opts["append"].'</span>';
			}
			$out .= '</div>';
		} else {
			$out .= forward_static_call_array(array('Form', $type), $args);
